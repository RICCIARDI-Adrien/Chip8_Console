...
unsigned char SDCardWaitForR1Response(void)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Received byte : 0x%02X.", Byte);
...
unsigned char SDCardProbe(void)
{
	...
	LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending CMD0 to the SD card...");
	SPI_SELECT_SD_CARD();
	Command_Argument = 0;
	...
	if (Result != 0x01)
	{
		if (Result == 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout during execution of CMD0.");
		else LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : initialization failed (R1 response : 0x%02X).", Result);
		return 1;
	}
	LOG(SD_CARD_IS_LOGGING_ENABLED, "CMD0 was successful.");
	__delay_ms(10);

	// Send the CMD8 (SEND_IF_COND) to determine whether the card is first generation or V2.00
	LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending CMD8 to the SD card...");
	SPI_SELECT_SD_CARD();
	...
	if (Result == 0x01)
	{
		...
		LOG(SD_CARD_IS_LOGGING_ENABLED, "The card supports version 2.00 or later of the specifications (R1 response : 0x%02X).", Result);
	}
	else if (Result == 0x05) // The CMD8 does not exist in protocol version 1.x, so the "illegal command" error is returned by the card
	{
		...
		LOG(SD_CARD_IS_LOGGING_ENABLED, "The card supports version 1.x of the specifications (R1 response : 0x%02X).", Result);
	}
	else
	{
		SPI_DESELECT_SD_CARD();
		if (Result == 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout during execution of CMD8.");
		else LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : the card does not support version 1.x nor version 2.00 of the specifications (R1 response : 0x%02X).", Result);
		return 1;
	}
	...
	if (SD_Card_Is_Version_2_Protocol)
	{
		...
		if (!(Buffer[2] & 0x01))
		{
			LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : the card did not confirm the voltage range.");
			return 1;
		}
		if (Buffer[3] != 0xAA)
		{
			LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : the check pattern returned by the card is wrong.");
			return 1;
		}
	}
	LOG(SD_CARD_IS_LOGGING_ENABLED, "CMD8 was successful.");
	__delay_ms(10);

	// Run the card initialization process until the card is ready
	LOG(SD_CARD_IS_LOGGING_ENABLED, "Looping through the initialization process...");
	for (i = 0; i < 100; i++) // The minimum timeout specified by the specifications is 1s
	{
		...
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending CMD55 to the SD card...");
		SPI_SELECT_SD_CARD();
		Command_Argument = 0;
		...
		{
			if (Result == 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout during execution of CMD8.");
			else LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : R1 response : 0x%02X.", Result);
			return 1;
		}
		LOG(SD_CARD_IS_LOGGING_ENABLED, "CMD55 was successful.");
		__delay_ms(10);

		// Run the initialization process
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending ACMD41 to the SD card...");
		SPI_SELECT_SD_CARD();
		...
		{
			LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : R1 response : 0x%02X.", Result);
			return 1;
		}
		if (Result == 0) // The "idle" bit is cleared when the initialization process is completed
		{
			LOG(SD_CARD_IS_LOGGING_ENABLED, "ACMD41 was successful.");
			break;
		}
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Card is not ready yet (attempt %d).", i + 1);
	...
	if (SD_Card_Is_Version_2_Protocol)
	{
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending CMD58 to the SD card...");
		SPI_SELECT_SD_CARD();
		Command_Argument = 0;
		...
		{
			SPI_DESELECT_SD_CARD();
			if (Result == 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout during execution of CMD58.");
			else LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : R1 response : 0x%02X.", Result);
			return 1;
		}
		...
		if (Buffer[0] & 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "OCR card power up status bit is set, card is ready.");
		else
		{
			LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : OCR card power up status bit is cleared, card is busy.");
			return 1;
		}
		// Card Capacity Status (bit 30), this bit is valid only if the Card power up status bit is set
		if (Buffer[0] & 0x40) LOG(SD_CARD_IS_LOGGING_ENABLED, "Card is High Capacity or Extended Capacity (CCS bit is set).");
		else LOG(SD_CARD_IS_LOGGING_ENABLED, "Card is Standard Capacity.");
		LOG(SD_CARD_IS_LOGGING_ENABLED, "CMD58 was successful.");
		__delay_ms(10);
	}
...
unsigned char SDCardReadBlock(unsigned long Block_Address, unsigned char *Pointer_Buffer)
{
	...
	LOG(SD_CARD_IS_LOGGING_ENABLED, "Sending CMD17 to the SD card (block address : %lu)...", Block_Address);
	SPI_SELECT_SD_CARD();
	...
	{
		SPI_DESELECT_SD_CARD();
		if (Result == 0x80) LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout during execution of CMD17.");
		else LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : R1 response : 0x%02X.", Result);
		return 1;
	}
	...
	{
		SPI_DESELECT_SD_CARD();
		LOG(SD_CARD_IS_LOGGING_ENABLED, "Error : timeout while waiting for the start token.");
		return 1;
	}
