...
void SPIInitialize(void)
{
	...
	SPI1CON1 = 0x04; // Sample SDI input in the middle of the data output time to let the time to the signal to stabilize, select clock edge and phase 0 mode, slave select pin is active low, SDI and SDO pins are active high
	...
	SPISetTargetDevice(SPI_DEVICE_ID_SD_CARD); // Configure the /SS pins, RC0 and RC1
...
{
	// RC0 is connected to the SD card
	if (Device_ID == SPI_DEVICE_ID_SD_CARD)
	{
		// Deselect the display
		LATCbits.LATC1 = 1;
		RC1PPS = 0; // Connect the latch function to the pin
